...
} from '/var/task/Dev/site-pv-temp/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/Dev/site-pv-temp/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/Dev/site-pv-temp')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/Dev/site-pv-temp',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/Dev/site-pv-temp/.netlify/dist/run/handlers/server.js')
